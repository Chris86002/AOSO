...
GLOBAL AOSO_UI2_MSN_TITLE IS 0.
GLOBAL AOSO_UI2_MSN_PHASE IS 0.
GLOBAL AOSO_UI2_MSN_ROUTE IS LIST().
GLOBAL AOSO_UI2_MSN_PROGRESS IS 0.
GLOBAL AOSO_UI2_MSN_BOTTOM IS 0.

FUNCTION aoso_ui2_build_mission_display {
    PARAMETER page.

    SET AOSO_UI2_MSN_TITLE TO page:ADDLABEL("<b><size=18>MISSION CONTROL</size></b>").
    SET AOSO_UI2_MSN_TITLE:STYLE:ALIGN TO "center".
    SET AOSO_UI2_MSN_TITLE:STYLE:HSTRETCH TO TRUE.
    ...
    SET AOSO_UI2_MSN_PHASE TO aoso_ui2_overlay_label(AOSO_UI2_MSN_MAIN, "", 22, 18).

    SET AOSO_UI2_MSN_ROUTE TO LIST().
    LOCAL row IS AOSO_UI2_MSN_MAIN:ADDHLAYOUT().
    SET row:STYLE:WIDTH TO 390.
    SET row:STYLE:HEIGHT TO 1.
    SET row:STYLE:MARGIN:H TO 15.
    SET row:STYLE:MARGIN:V TO 78.
    LOCAL i IS 0.
    UNTIL i >= 5 {
        LOCAL lab IS row:ADDLABEL("---").
        SET lab:STYLE:WIDTH TO 78.
        SET lab:STYLE:ALIGN TO "center".
        AOSO_UI2_MSN_ROUTE:ADD(lab).
        SET i TO i + 1.
    }

    SET AOSO_UI2_MSN_BOTTOM TO aoso_ui2_overlay_label(AOSO_UI2_MSN_MAIN, "", 22, 132).

    LOCAL prow IS page:ADDHLAYOUT().
    SET prow:STYLE:WIDTH TO 420.
    LOCAL plab IS prow:ADDLABEL("TOUR").
    SET plab:STYLE:WIDTH TO 42.
    SET AOSO_UI2_MSN_PROGRESS TO prow:ADDHSLIDER(0, 0, 1).
    SET AOSO_UI2_MSN_PROGRESS:STYLE:WIDTH TO 290.
    SET AOSO_UI2_MSN_PROGRESS:STYLE:HEIGHT TO 18.
    SET AOSO_UI2_MSN_PROGRESS:STYLE:HSTRETCH TO FALSE.
    SET AOSO_UI2_MSN_PROGRESS:STYLE:VSTRETCH TO FALSE.
    aoso_hud_lab(prow, "ui2_msn_pct", "0%").

    aoso_hud_lab(page, "ui2_msn_objective", "").
    aoso_hud_lab(page, "ui2_msn_assurance", "").
}
...
FUNCTION aoso_ui2_mission_update {
    ...
    IF phase = "" { SET phase TO "STANDBY". }

    SET AOSO_UI2_MSN_TITLE:TEXT TO "<b><size=18>MISSION  " + current + "</size></b>".
    SET AOSO_UI2_MSN_PHASE:TEXT TO "<b>" + phase + "</b>" + CHAR(10) +
        "NOW  " + SHIP:BODY:NAME + "    NEXT  " + m["next"].

    LOCAL frac IS 0.
    IF m["n"] > 0 {
        SET frac TO CLAMP((m["idx"] + 1) / m["n"], 0, 1).
    }
    SET AOSO_UI2_MSN_PROGRESS:VALUE TO frac.
    aoso_hud_set("ui2_msn_pct", ROUND(frac * 100, 0) + "%").
    ...
    IF DEFINED AOSO_TOUR {
        IF AOSO_TOUR:HASKEY("data") {
            IF AOSO_TOUR["data"]:HASKEY("targets") {
                SET targets TO AOSO_TOUR["data"]["targets"].
                SET idx TO AOSO_TOUR["data"]["index"].
            }
        }
    }

    LOCAL j IS 0.
    UNTIL j >= AOSO_UI2_MSN_ROUTE:LENGTH {
        LOCAL target_i IS idx + j - 2.
        LOCAL txt IS "—".
        IF target_i >= 0 AND target_i < targets:LENGTH {
            LOCAL nm IS targets[target_i].
            IF target_i < idx {
                SET txt TO "<color=#6E8C78>✓ " + nm + "</color>".
            } ELSE {
                IF target_i = idx {
                    SET txt TO "<color=#50FF96><b>[" + nm + "]</b></color>".
                } ELSE {
                    IF target_i = idx + 1 {
                        SET txt TO "<color=#7EC8FF>" + nm + "</color>".
                    } ELSE {
                        SET txt TO nm.
                    }
                }
        ...
        SET AOSO_UI2_MSN_ROUTE[j]:TEXT TO txt.
        SET j TO j + 1.
    }

    LOCAL cert IS m["cert"].
    IF cert = "" { SET cert TO "—". }
    LOCAL health IS m["health"].
    IF health = "" { SET health TO AOSO_HUD_DATA["systems"]["rollup"]. }
    SET AOSO_UI2_MSN_BOTTOM:TEXT TO "dV " + ROUND(res["mission_dv"], 0) +
        " m/s   CERT " + cert + CHAR(10) + "ASSURANCE " + health.

    LOCAL objective IS AOSO_HUD_DATA["flight"]["doing"].
    IF objective = "" { SET objective TO m["step"]. }
    aoso_hud_set("ui2_msn_objective", "OBJECTIVE  " + objective).

    LOCAL assure IS "FEAS " + m["feas"].
    IF m["weak"] <> "" { SET assure TO assure + "   WEAK " + m["weak"]. }
    IF m["skip"] <> "" { SET assure TO aoso_ui2_color_state("WARN", m["skip"]). }
    aoso_hud_set("ui2_msn_assurance", assure).
}

// -------------------------------------------------------------------------
// Systems / caution and warning panel
// -------------------------------------------------------------------------
GLOBAL AOSO_UI2_SYS_MAIN IS 0.
GLOBAL AOSO_UI2_SYS_CELLS IS LIST().
GLOBAL AOSO_UI2_SYS_MASTER IS 0.
GLOBAL AOSO_UI2_SYS_WHY IS 0.

FUNCTION aoso_ui2_sys_bg {
    PARAMETER state.
    IF state = "FAIL" { RETURN AOSO_UI2_ASSET_ROOT + "button_fail.png". }
    IF state = "DEG" { RETURN AOSO_UI2_ASSET_ROOT + "button_warn.png". }
    IF state = "NOM" { RETURN AOSO_UI2_ASSET_ROOT + "button_on.png". }
    RETURN AOSO_UI2_ASSET_ROOT + "button_stby.png".
}

FUNCTION aoso_ui2_build_systems_display {
    PARAMETER page.

    LOCAL title IS page:ADDLABEL("<b><size=18>SYSTEMS / ANNUNCIATOR PANEL</size></b>").
    SET title:STYLE:ALIGN TO "center".
    SET title:STYLE:HSTRETCH TO TRUE.
    ...
    SET AOSO_UI2_SYS_CELLS TO LIST().
    LOCAL names IS LIST(
        "GUIDANCE", "NAV", "STEERING",
    ...
    UNTIL row_i >= 3 {
        ...
        UNTIL col >= 3 {
            ...
            SET lab:STYLE:ALIGN TO "center".
            SET lab:STYLE:BG TO AOSO_UI2_ASSET_ROOT + "button_stby.png".
    ...
    aoso_hud_lab(page, "ui2_sys_cpu", "").
    SET AOSO_UI2_SYS_WHY TO page:ADDLABEL("").
    SET AOSO_UI2_SYS_WHY:STYLE:HSTRETCH TO TRUE.
}

FUNCTION aoso_ui2_systems_update {
    LOCAL s IS AOSO_HUD_DATA["systems"].
    LOCAL keys IS LIST("guid", "nav", "steer", "thr", "stg", "msn", "lnd", "pwr", "com").
    LOCAL names IS LIST("GUIDANCE", "NAV", "STEERING", "THROTTLE", "STAGING", "MISSION", "LANDING", "POWER", "COMMS").

    LOCAL i IS 0.
    UNTIL i >= keys:LENGTH {
        LOCAL st IS s[keys[i]].
        SET AOSO_UI2_SYS_CELLS[i]:TEXT TO names[i] + CHAR(10) + st.
        SET AOSO_UI2_SYS_CELLS[i]:STYLE:BG TO aoso_ui2_sys_bg(st).
        SET i TO i + 1.
    }

    LOCAL master IS "SYSTEMS NOMINAL".
    LOCAL mast_state IS "NOM".
    IF s["worst"] = "DEG" {
        SET master TO "MASTER CAUTION".
        SET mast_state TO "WARN".
    }
    IF s["worst"] = "FAIL" {
        SET master TO "MASTER WARNING".
        SET mast_state TO "FAIL".
    }
    SET AOSO_UI2_SYS_MASTER:TEXT TO aoso_ui2_color_state(mast_state, master).

    LOCAL cpu IS AOSO_HUD_DATA["debug"]["cpu"].
    LOCAL band IS "".
    IF AOSO_HUD_DATA["debug"]:HASKEY("band") { SET band TO AOSO_HUD_DATA["debug"]["band"]. }
    aoso_hud_set("ui2_sys_cpu", "CPU " + cpu + " " + band +
        "   EC " + ROUND(AOSO_HUD_DATA["res"]["ec"], 0) + "%   WD " + s["wd"]).

    LOCAL why IS s["why"].
    IF why = "" { SET why TO "All monitored flight systems nominal.". }
    SET AOSO_UI2_SYS_WHY:TEXT TO "WHY  " + why.
}

// -------------------------------------------------------------------------
// Graphical digital twin stage/resource schematic
// -------------------------------------------------------------------------
GLOBAL AOSO_UI2_TWIN_ROWS IS LIST().
GLOBAL AOSO_UI2_TWIN_TITLE IS 0.
GLOBAL AOSO_UI2_TWIN_SUMMARY IS 0.

FUNCTION aoso_ui2_build_twin_display {
    PARAMETER frame.

    SET frame:STYLE:BG TO AOSO_UI2_ASSET_ROOT + "twin_frame.png".
    SET AOSO_UI2_TWIN_TITLE TO aoso_ui2_overlay_label(frame, "", 24, 17).

    SET AOSO_UI2_TWIN_ROWS TO LIST().
    LOCAL i IS 0.
    UNTIL i >= 7 {
        LOCAL row IS aoso_ui2_overlay_label(frame, "", 34, 48 + i * 25).
        SET row:STYLE:WIDTH TO 350.
        AOSO_UI2_TWIN_ROWS:ADD(row).
        SET i TO i + 1.
    }

    SET AOSO_UI2_TWIN_SUMMARY TO aoso_ui2_overlay_label(frame, "", 34, 224).
}

FUNCTION aoso_ui2_twin_band_summary {
    PARAMETER band.
    LOCAL stage IS -99.
    LOCAL eng_n IS 0.
    LOCAL eng_on IS 0.
    ...
    FOR node IN band {
        SET mass TO mass + node["mass"].
        IF stage = -99 { SET stage TO node["decoupled_in"]. }
        IF node["kind"] = "engine" {
            SET eng_n TO eng_n + 1.
            IF node["ignition"] AND NOT node["flameout"] { SET eng_on TO eng_on + 1. }
        }
        FOR rr IN node["resources"] {
            IF aoso_capabilities_is_propellant(rr["name"]) {
                SET prop_amt TO prop_amt + rr["amount"].
                SET prop_cap TO prop_cap + rr["capacity"].
            }
        }
    }

    LOCAL pct IS 0.
    IF prop_cap > 0.001 { SET pct TO prop_amt / prop_cap. }
    LOCAL st_txt IS "S" + stage.
    IF stage < 0 { SET st_txt TO "CORE". }

    RETURN st_txt + "   ENG " + eng_on + "/" + eng_n + "   PROP " +
        aoso_ui2_bar(pct, 14) + " " + ROUND(pct * 100, 0) + "%   " +
        ROUND(mass, 1) + "t".
}
...
FUNCTION aoso_ui2_twin_update {
    ...
    SET AOSO_UI2_TWIN_TITLE:TEXT TO "<b>" + AOSO_TWIN["status"] + "</b>   " +
        AOSO_TWIN["part_n"] + " parts   STAGE " + STAGE:NUMBER.

    LOCAL i IS 0.
    UNTIL i >= AOSO_UI2_TWIN_ROWS:LENGTH {
        IF i < bands:LENGTH {
            LOCAL txt IS aoso_ui2_twin_band_summary(bands[i]).
            IF bands[i]:LENGTH > 0 {
                IF bands[i][0]["decoupled_in"] = STAGE:NUMBER {
                    SET txt TO "<color=#50FF96><b>▶ " + txt + "</b></color>".
                }
            }
            SET AOSO_UI2_TWIN_ROWS[i]:TEXT TO txt.
            SET AOSO_UI2_TWIN_ROWS[i]:VISIBLE TO TRUE.
        } ELSE {
            SET AOSO_UI2_TWIN_ROWS[i]:VISIBLE TO FALSE.
        }
        SET i TO i + 1.
    }

    LOCAL limited_txt IS "".
    IF AOSO_TWIN["limited"] { SET limited_txt TO "  LIMITED VIEW". }
    SET AOSO_UI2_TWIN_SUMMARY:TEXT TO "ENG " + AOSO_TWIN["engines_on"] + "/" +
        AOSO_TWIN["engines_total"] + "   ROOT " + AOSO_TWIN["root_uid"] + limited_txt.
}
