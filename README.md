...
Burns lock facing at ignition and feather throttle against remaining burn-time (not a fixed 2 m/s cutoff), so they no longer chase the live node marker. The gravity turn follows surface prograde with a small angle-of-attack limit instead of a raw cosine pitch program.
